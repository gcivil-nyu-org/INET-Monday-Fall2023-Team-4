...
class NotificationListView(View):
    ...
    def get(self, request):
        object_list = Notification.objects.filter(Q(transferownershipnotif__new_owner=self.request.user, transferownershipnotif__status="pending") | Q(transferownershipnotif__original_owner=self.request.user, transferownershipnotif__status="declined") | Q(bookclubupdatesnotif__receiving_user=self.request.user))
        
        for n in object_list:
            n.is_read = True
            n.save()
        return render(request, "notifications/notifications.html", {"notifs": object_list})
        ...
        if notif_type == 'transfer':
            ...
            if req_status:
                if req_status == "Accept":
                    ...
                    updateNotif = BookClubUpdatesNotif(safe_to_delete=True, date_created=date.today(), receiving_user=request.user, book_club=book_club, fields_changed='new admin')
                    updateNotif.save()
                else:
